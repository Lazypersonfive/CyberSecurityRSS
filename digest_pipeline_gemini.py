...
BOARD_SCORE_SYSTEM = {
    "security": """你是资深网安编辑，对安全资讯做 0-10 打分。目标是技术日报，不是泛安全新闻。
评分标准：
- 9-10: 最新热门 CVE / 0day / 在野利用，且能说明漏洞原理、触发条件、影响版本或利用方式；中文漏洞分析、国内官方预警、厂商应急通告、CTF/攻防竞赛高质量技术复盘
- 7-8: 高危漏洞详解、补丁绕过、PoC/EXP 分析、供应链技术细节、红蓝对抗工具与检测规则
- 5-6: 安全研究、技术博客、官方通告、工具更新；必须有明确技术信息或可执行处置价值
...
5. 如果是 CVE、漏洞、PoC、EXP、补丁绕过或官方预警，摘要必须优先写：漏洞类型/原理、触发条件、影响版本或影响组件、实际影响；原文缺哪项就明确写原文未披露。
6. 如果是 CTF/竞赛/红蓝对抗/工具文章，摘要必须写技术点、适用场景和为什么值得安全从业者关注。
7. 不要把地缘政治归因当成重点；没有技术细节的 APT/黑客组织新闻应写得很弱，不得拔高。
8. tags 给 1-3 个中文关键词，每个不超过 6 字；selection_reason 不得为空，用不超过 30 个中文字符说明技术价值。
