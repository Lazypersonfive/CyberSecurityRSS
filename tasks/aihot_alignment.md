...
## Public RSS Feeds
AIHOT also exposes standard RSS feeds. Use these for our lightweight external benchmark before using the REST API.

| Feed | URL | Use |
|---|---|---|
| 精选 | `https://aihot.virxact.com/feed.xml` | Default benchmark for missed AI热点; current `aihot_compare.py` default. |
| 全部 AI 动态 | `https://aihot.virxact.com/feed/all.xml` | Manual broad audit only; noisier, not a production source. |
| AI 日报 | `https://aihot.virxact.com/feed/daily.xml` | Daily-view comparison, not item-level source ingestion. |

Policy remains unchanged: AIHOT is an external benchmark, not a primary source. If it ever enters production feeds, it must be marked as external/aggregated and capped below official sites and official X.
